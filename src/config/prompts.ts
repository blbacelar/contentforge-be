import type { PromptDefinitions } from '../types/prompts';

export const prompts: PromptDefinitions = {
  captions: {
    system: (lang, count, tone, niche) => 
...
Watch this space. What are your thoughts on this pivotal moment in migration policy? ⚖️🌊 #RwandaPolicy #AsylumSeekers #MigrationDebate #UKParliament #HumanDignity #InternationalLaw #ModernSlavery #GlobalMigration #ChannelCrossings #UKGovernment #PoliticalEditorial #EthicalPolicy #PublicAccountability #LegislativeChallenges #LawAndOrder #NationalSecurity #SafeHarbours #VoteOutcome`,
    user: (content, count, tone, niche) => 
      `Content:\n${content}\nTone: ${tone}\nNiche: ${niche}\n\nGenerate ${count} captions numbered 1-${count}:`
  },
  script: {
    system: (lang, tone, niche) => 
...
Use this exact structure:
    ...
    user: (content, tone, niche) => 
      `Content:\n${content}\nTone: ${tone}\nNiche: ${niche}\n\nGenerate script JSON:`
  },
  summary: {
    system: (lang, tone, niche) => `Generate summary in ${lang} with ${tone} tone for ${niche}...`,
    user: (content, tone, niche) => 
      `Content: ${content}\nTone: ${tone}\nNiche: ${niche}`
  },
  expertise: {
    system: (lang, tone, niche) => `Identify expertise areas in ${lang} with a ${tone} tone for the ${niche} domain:`,
    user: (content, tone, niche) => `Analyze: ${content}\nTone: ${tone}\nNiche: ${niche}`
  },
  variation: {
    system: (lang, tone, niche) => `Create variations in ${lang} with a ${tone} tone for ${niche}:`,
    user: (content, tone, niche) => `Original: ${content}\nTone: ${tone}\nNiche: ${niche}`
  }
} as const;
